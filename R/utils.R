  ...
  if(length(idx)>0){
    for(i in seq(1, length(idx), by=2)){
      ...
      if((module1 %in% colnames(data)) && (module2 %in% colnames(data))){
        colors[which(colors%in%c(substring(module1, 3), substring(module2, 3)))] = substring(module1, 3) #Rename colors with one of the two modules correlated
        data <- data %>%
          dplyr::mutate(new_column = rowMeans(data[,c(module1, module2)])) %>%
          dplyr::select(., -all_of(c(module1, module2))) %>%
