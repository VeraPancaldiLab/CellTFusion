...
#' Compute survival analysis using CoxPH and Kaplan-Meier Models
#'
#' This function performs survival analysis by fitting univariate and multivariate Cox proportional hazards (CoxPH) models on combinations of the input features. It uses the linear predictors from the CoxPH model to stratify patients into high- and low-risk groups based on a threshold (default the median). Kaplan-Meier survival curves are then generated for each group, and a log-rank test is performed to assess statistical significance. Survival curves are saved as PDF files under the `Results/` directory.
#'
#' @param features A numeric matrix or data frame of cell group or module scores. Rows correspond to samples and columns to different features.
#' @param survival.data A data frame containing survival information. Must include:
#' \describe{
#'   \item{PFS_Days}{Numeric vector of progression-free survival times.}
#'   \item{PFS_Outcome}{Binary vector (0/1) indicating progression/recurrence/death (1 = event, 0 = censored).}
#' }
#' @param time_unit A string indicating the time unit used in survival analysis (e.g., `"days"`, `"months"`, `"years"`). This is used for x-axis labeling in Kaplan-Meier plots.
#' @param p.value Significance threshold for the log-rank test p-value. Only combinations with p-value below this threshold will be considered significant. Default is `0.05`.
#' @param thres Threshold used to binarize the risk scores into high and low groups. Default is `0.5`, corresponding to the median.
#' @param max_factors Maximum number of features to include in a model. Default is `Inf`, meaning all combinations will be considered.
#'
#' @return A named list of significant model formulas (as `formula` objects), with corresponding Kaplan-Meier plots saved as PDFs in the `Results/` folder.
#'
#' @export
#'
compute.survival.analysis = function(features, survival.data, time_unit, p.value = 0.05, thres = 0.5, max_factors = Inf) {
  ...
  for (n in 1:min(n_features, max_factors)) {
    ...
    for (comb in combinations) {
      ...
      formula <- stats::as.formula(paste("Surv(time, status) ~", paste(colnames(features)[comb], collapse = " + ")))

      # Prepare the data for survival analysis
      data_for_model <- data.frame("time" = survival.data$PFS_Days,
                                   "status" = survival.data$PFS_Outcome)
...
#'
#' @return A data.frame with added subgroup features (based on median of constituent features)
#' @export
#'
