...
CellTFusion latent factors (`res$Latent_spaces$Z`) serve as features for
machine learning models. This article shows how to train a classifier on
a training cohort and project it onto an independent test cohort using
...
> [`pipeML`](https://github.com/VeraPancaldiLab/pipeML) package for
> model training. Install it from GitHub before running these examples:
>
> ``` r
>
...
classifiers (random forest, elastic net, etc.) and returns AUROC
results.
...
[`project_test_factors()`](https://verapancaldilab.github.io/CellTFusion/reference/project_test_factors.md)
takes the full training result object (`res_train`) and the
deconvolution of the test cohort, and projects the test samples into the
training latent space. This ensures no data leakage from the test set
...
`compute_features_modular()` pattern from the paper scripts wraps
...
This pattern ensures the TF module structure is learned only from
training samples and consistently applied to test samples at each fold.
